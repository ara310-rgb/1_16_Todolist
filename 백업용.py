...
st.markdown("""
    ...
    .feature-box:hover { transform: translateY(-10px); box-shadow: 0 15px 30px rgba(0, 123, 255, 0.2); border-color: #007bff; }
    ...
    /* --- [섹션 6: 안내 사항 디자인 수정 - 흰색 배경 및 연한 음영] --- */
    .notice-box {
        background-color: #ffffff; /* 배경을 흰색으로 변경 */
        border-left: 5px solid #e2e8f0; /* 왼쪽 테두리 색상을 더 부드럽게 변경 */
        padding: 22px 28px;
        border-radius: 12px;
        margin-top: 25px;
        /* 연한 회색 톤의 부드러운 음영 (왼쪽 제외) */
        box-shadow: 4px 4px 15px rgba(0, 0, 0, 0.04), 
                    -1px -1px 10px rgba(0, 0, 0, 0.02);
    }
    .notice-title {
        font-size: 16px;
        font-weight: 700;
        color: #94a3b8; /* 조금 더 연한 그레이 블루 */
        margin-bottom: 10px;
        display: flex;
        align-items: center;
        gap: 6px;
...
if st.session_state.page == "home":
    ...
    # [섹션 4: 핵심 서비스 장점]
    st.markdown("<br><h3 style='text-align:center;'>왜 수출역량진단이 필요한가요?</h3>", unsafe_allow_html=True)
    st.markdown('''
    ...
    <div class="fixed-layout-container">
        <h3 style="text-align:center; margin-bottom: 20px;">서비스 이용 방법</h3>
